...
def get_age_distribution_interpretation(filtered_df):
    """Generate interpretation for age distribution."""
    female_mean = filtered_df[filtered_df["Sex"] == "F"]["Age"].mean()
    male_mean = filtered_df[filtered_df["Sex"] == "M"]["Age"].mean()
    age_diff = abs(female_mean - male_mean)
    
    interpretation = f"""
    ### Key Findings in Age Distribution:
    
    1. **Gender Comparison**:
       - Female athletes average age: {female_mean:.1f} years
       - Male athletes average age: {male_mean:.1f} years
       - Age gap between genders: {age_diff:.1f} years
    
    2. **Distribution Pattern**:
       - {"Women tend to peak earlier than men" if female_mean < male_mean else "Men tend to peak earlier than women"}
       - {"The age difference is significant" if age_diff > 3 else "The age difference is minimal"}
       
    3. **Performance Implications**:
       - Peak performance age range: {filtered_df['Age'].quantile(0.25):.0f}-{filtered_df['Age'].quantile(0.75):.0f} years
       - This suggests the optimal competitive window for Olympic archery
    """
    return interpretation

def get_medal_distribution_interpretation(filtered_df):
    """Generate interpretation for medal distribution."""
    age_medals = filtered_df.groupby("Age_Group")["Medal"].value_counts().unstack()
    most_successful = age_medals.sum(axis=1).idxmax()
    gold_age = age_medals["Gold"].idxmax() if "Gold" in age_medals else "N/A"
    
    interpretation = f"""
    ### Medal Distribution Insights:
    
    1. **Age Group Success**:
       - Most successful age group: {most_successful}
       - Highest gold medal concentration: {gold_age}
    
    2. **Performance Patterns**:
       - {"Younger athletes (Under 25) show strong medal presence" if "Under 20" in most_successful or "20-25" in most_successful else "Experienced athletes dominate the medal counts"}
       - Peak performance appears to be in the {most_successful} range
       
    3. **Competition Level**:
       - The data suggests that {most_successful} athletes have the best medal-winning probability
       - This could influence training and athlete development strategies
    """
    return interpretation

def get_trend_interpretation(filtered_df):
    """Generate interpretation for age trends over time."""
    yearly_avg = filtered_df.groupby("Year")["Age"].mean()
    ...
    1. **Historical Progression**:
       - Overall trend: Age is {trend} over time
       - Average age change: {(yearly_avg.iloc[-1] - yearly_avg.iloc[0]):.1f} years
    
    2. **Evolution of the Sport**:
       - {"The sport is trending towards more experienced athletes" if trend == "increasing" else "The sport is becoming more accessible to younger athletes"}
       - This could reflect changes in training methods and technology
       
    3. **Future Implications**:
       - The trend suggests continued {trend} average ages
       - This may influence recruitment and development strategies
    """
    return interpretation
...
with col1:
    ...
    selected_year = st.selectbox("Select Olympic Year", ["All"] + list(years), index=0)
with col2:
    selected_gender = st.radio("Select Gender", ["Both", "M", "F"], horizontal=True)
with col3:
    event_types = df["Event_Type"].unique()
    selected_event = st.selectbox("Select Event Type", ["All"] + list(event_types))
with col4:
    seasons = df["Season"].unique()
    selected_season = st.selectbox("Select Season", ["All"] + list(seasons))
...
# Display visualizations with interpretations
st.markdown("### 🎯 Age Distribution Analysis")
fig_dist = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.02, row_heights=[0.5, 0.5])
...
fig_dist.add_trace(
    go.Bar(x=female_df["Age"].value_counts().sort_index().index,
           ...
           name="Female", marker_color="#FF69B4"),
    row=1, col=1
)
fig_dist.add_trace(
    go.Bar(x=male_df["Age"].value_counts().sort_index().index,
           y=-male_df["Age"].value_counts().sort_index().values,
           name="Male", marker_color="#4169E1"),
    row=2, col=1
)

fig_dist.update_layout(height=600, showlegend=True, title_text="Age Distribution by Gender")
st.plotly_chart(fig_dist, use_container_width=True)
st.markdown(get_age_distribution_interpretation(filtered_df), unsafe_allow_html=True)

# Add remaining visualizations and interpretations
st.markdown("### 📈 Performance Trends")
age_trends = px.box(filtered_df, x="Year", y="Age", color="Sex", title="Age Distribution Across Olympics")
st.plotly_chart(age_trends, use_container_width=True)
st.markdown(get_trend_interpretation(filtered_df), unsafe_allow_html=True)
...
medal_dist = px.bar(medal_counts, x="Age_Group", y="count", color="Medal",
...
st.markdown(get_medal_distribution_interpretation(filtered_df), unsafe_allow_html=True)
